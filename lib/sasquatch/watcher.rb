...
class Watcher
  ...
  def start
    @supposend_to_be_watching = true
    while  @supposed_to_be_watching

      haml_changes = haml_changes_in(@haml_folder)
      transform_haml(haml_changes, @html_folder, @haml_folder) unless haml_changes.empty?

      sass_changes = sass_changes_in(@sass_folder)
      transform_sass(sass_changes, @css_folder, @sass_folder) unless sass_changes.empty?
